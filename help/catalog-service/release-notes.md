...
![Neu](../assets/new.svg) Die [[!DNL Data Management Dashboard]](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/data-transfer/data-sync/data-dashboard) ist jetzt für Datenströme verfügbar (Produktempfehlungen, Live-Suche, Katalog-Service). Erfordert `catalog-service` Metapaket v3.1.0+.
...
![Neu](../assets/new.svg) **Export anpassbarer Optionen** - Die Abhängigkeiten des Metapakets für den Katalog-Service wurden aktualisiert, sodass sie das Modul „Datenexporteur für anpassbare Optionen“ (`magento/module-customizable-options-data-exporter`) enthalten. Dieses Modul exportiert die anpassbaren Adobe Commerce-Optionen eines Produkts - sowohl auswählbare Optionen (Dropdown, Optionsschaltfläche, Kontrollkästchen, Mehrfachauswahl) als auch Einkäufer-Eingabeoptionen (Textfeld, Textbereich, Datei, Datum, Datum und Uhrzeit, Uhrzeit) -, sodass sie an den SaaS-Katalog weitergegeben werden, der vom Katalog-Service genutzt wird. <!--DATA-7366-->
...
- Informationen zu Projekten, die in **Adobe Commerce in der Cloud, lokal oder in Adobe Commerce as a Cloud Service bereitgestellt werden, finden Sie in der folgenden Dokumentation:
  ...
  - [Adobe Commerce-Administratorhandbuch](https://experienceleague.adobe.com/en/docs/commerce-admin/user-guides/home)
  - [Handbuch zu Adobe Commerce as a Cloud Service](../cloud-service/overview.md)
  - [Handbuch zu Adobe Commerce in Cloud Manager](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/overview)
