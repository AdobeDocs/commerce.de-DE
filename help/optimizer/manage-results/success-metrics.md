...
**Datengenauigkeit:** Metriken werden aus tatsächlichen Kundeninteraktionen berechnet, die durch Storefront-Ereignisse erfasst werden. Stellen Sie sicher, dass Ihr Store über eine ordnungsgemäße [Ereignisverfolgung konfiguriert](../setup/events/overview.md) für ein genaues Reporting verfügt.
...
Erfolgsmetriken und Sites Optimizer ([Opportunities](opportunities.md)) sind komplementäre Tools, die gemeinsam entwickelt wurden, um die Leistung Ihrer Commerce-Site zu verbessern. Wenn Sie den Unterschied zwischen diesen Funktionen verstehen, können Sie bessere Entscheidungen treffen und messbare Ergebnisse erzielen.

### Die wichtigsten Unterschiede

| Aspekt | Erfolgsmetriken | Sites Optimizer (Chancen) |
|---|---|---|
| **Zweck** | Misst Leistung und Ergebnisse | Identifiziert Probleme und gibt Empfehlungen |
| **Typ** | Analytics-Dashboard | Proaktive Problemerkennung |
| **Was es zeigt** | Wichtige Leistungsindikatoren (Umsatz, Konversion, Interaktion, Akquise, Absprungrate) | KI-gestützte Empfehlungen für Probleme, die die Site-Leistung beeinträchtigen |
| **Datenquelle** | Storefront-Ereignisdaten | Produktkataloge, Suchprotokolle, Empfehlungsdaten |
| **Verwenden Sie wenn** | Ergebnisse sollen im Zeitverlauf verfolgt werden | Sie möchten spezifische Probleme identifizieren und beheben |
...
1. **Mit Erfolgsmetriken messen**: Überprüfen Sie zunächst Ihr Dashboard für Erfolgsmetriken, um Ihre aktuelle Leistung zu verstehen. Ermitteln Sie, welche KPIs verbessert werden müssen (z. B. niedrige Konversionsrate oder hohe Absprungrate).

1. **Diagnose mit Opportunities**: Navigieren Sie zur Seite „Opportunitys“, um spezifische Probleme zu identifizieren, die möglicherweise zu einer schlechten Leistung führen. Sites Optimizer scannt Ihren Produktkatalog, Suchprotokolle und Empfehlungsdaten, um Probleme wie fehlende Produktdaten, mangelhafte Suchrelevanz oder Navigationsprobleme zu identifizieren.

1. **Implementieren von Empfehlungen**: Befolgen Sie die KI-gesteuerten Empfehlungen in Opportunities , um erkannte Probleme zu beheben. Dazu können die Behebung von Problemen mit der Qualität von Produktdaten, die Verbesserung von SEO oder die Optimierung von Suche und Erkennung gehören.

1. **Verbesserungen verfolgen**: Kehren Sie zu Erfolgsmetriken zurück, um zu überwachen, wie sich die Änderungen im Laufe der Zeit auf Ihre KPIs auswirken. Verwenden Sie die Datumsbereichsauswahl, um die Leistung vor und nach der Implementierung von Empfehlungen zu vergleichen.

1. **Iterieren und optimieren**: Setzen Sie diesen Zyklus fort und nutzen Sie Chancen zur Identifizierung neuer Probleme und Erfolgsmetriken, um die Wirkung Ihrer Optimierungen zu messen.
...
1. **Problem identifizieren**: Das Dashboard Erfolgsmetriken zeigt die Konversionsrate an, die im letzten Monat um 15 % gesunken ist.

1. **Ursache finden**: Auf der Seite „Opportunitys“ werden mehrere Probleme angezeigt:
...
1. **Maßnahmen ergreifen**: Der Händler räumt der Behebung von Problemen mit der Qualität der Produktdaten Priorität ein, da Sites Optimizer diese als wirkungsvolle Chancen einstuft, die sich auf die Suche und Empfehlungen auswirken.

1. **Messergebnisse**: Nach der Aktualisierung der Produktattribute und der Implementierung empfohlener Änderungen überwacht der Händler Erfolgsmetriken wöchentlich. Im Laufe des nächsten Monats steigt die Konversionsrate um 12 %, und die Metriken für die Suchinteraktion verbessern sich erheblich.

1. **Weiter optimieren**: Mit der Verbesserung der Konversionsrate verlagert der Händler den Fokus auf die nächste Priorität, die in Opportunities angezeigt wird - die Optimierung der Seitenladegeschwindigkeit, um die Absprungrate zu reduzieren.
...
**Datenquelle:** Storefront-Ereignisse (insbesondere `place-order` Ereignisse)
...
- **Produktansichten** werden verfolgt, wenn Besucher Produktseiten anzeigen (mithilfe von `product-view`).
- **Käufe** werden beim Abschluss von Bestellungen verfolgt (mithilfe von `place-order`).
- Die Berechnung gleicht Benutzer, die bestimmte Produkte angesehen haben, mit denen ab, die sie gekauft haben.
...
- Kategorie: `product`, `shopper`, `shopping-cart` oder `checkout`.
- Eigenschaft: `Product`, `Checkout`, `Cart`, `Category` oder `Wishlist`.
...
- Der Browser jedes Besuchers erhält über ein Erstanbieter-Cookie eine eindeutige Kennung (`domain_userid`).
- Neue Besucher werden identifiziert, wenn ihr Sitzungsindex 1 ist (erster Besuch).
...
![Erfolgsmetriken Bounce-Rate](../assets/bounce-rate.png)
...
**Was ist eine gute Absprungrate?** Dies variiert je nach Branche und Seitentyp, aber im Allgemeinen:
...
- [Recommendations-Performance](../manage-results/recommendation-performance.md) - Überwachen und Optimieren der Effektivität von Recommendations.
...
- [Facetten](../merchandising/facets/overview.md) - Verbessern Sie die Suche mit intelligenter Filterung.
- [Synonyme](../merchandising/synonyms/overview.md) - Verbessern Sie die Suchrelevanz und das Kundenerlebnis.
