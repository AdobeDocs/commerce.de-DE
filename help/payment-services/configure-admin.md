...
1. [Commerce Services-Connector](../landing/saas.md) Einrichtung ist abgeschlossen.

1. [Sandbox- und Produktions](connect.md#configure-commerce-services)-API-Schlüssel (öffentlich und privat) werden in der Admin Console gespeichert.

1. **[!UICONTROL Payment Services Sandbox ID]** und **[!UICONTROL Payment Services Production ID]** werden in &quot;[&#x200B; Konfiguration“ &#x200B;](#general-configuration).
...
Weitere Informationen finden [&#x200B; unter &#x200B;](payments-options.md#paypal-payment-buttons).
...
| [!UICONTROL Payment Action] | Website | Die [Zahlungsaktion](https://experienceleague.adobe.com/en/docs/commerce-admin/config/sales/payment-methods/payment-methods) für die angegebene Zahlungsmethode. Optionen: [!UICONTROL Authorize] / [!UICONTROL Authorize and Capture] |
...
Mit [!DNL Apple Pay] können Händler ein sicheres, schnelles und nahtloses Checkout-Erlebnis bieten - mit Unterstützung von bis zu 99 Domains pro Händlerkonto. In **Safari** (macOS und iOS) werden mit der Schaltfläche [!DNL Apple Pay] automatisch Zahlungs-, Kontakt- und Versandinformationen vom Gerät des Kunden ausgefüllt, sowohl am Anfang der Kasse (Express) als auch auf der endgültigen Kasse. In **Chrome, Firefox oder Microsoft Edge** ist [!DNL Apple Pay] sowohl während des **Express-Checkouts** als auch beim **letzten Checkout-Schritt** verfügbar. Auf Desktops ermöglichen ein QR-Code und ein **iPhone** (iOS 18 oder höher) dem Käufer, die Zahlung in der Apple-Pay-Tabelle abzuschließen. Stellen Sie sicher, dass **[!UICONTROL Show Apple Pay on product detail page]** oder andere Platzierungen an den Stellen aktiviert sind, an denen Sie einen Express-Checkout durchführen möchten.
...
1. Erweitern Sie im linken Bereich **[!UICONTROL Sales]** und wählen Sie **[!UICONTROL Payment Methods]**.
1. Erweitern Sie den Abschnitt _[!UICONTROL FEATURED ADOBE PAYMENT SOLUTION]_.
1. Erweitern Sie im Abschnitt _[!UICONTROL [!DNL Payment Services]]_&#x200B;den Abschnitt&#x200B;_[!UICONTROL Apple Pay]_ .
1. Geben Sie **[!UICONTROL Title]** Text ein (falls erforderlich), um den Namen der Zahlungsmethode wie beim Checkout angezeigt zu ändern.
...
| [!UICONTROL Title] | Shop-Ansicht | Fügen Sie den Text hinzu, der während des Checkouts als Titel für diese Zahlungsoption in der Ansicht „Zahlungsmethode“ angezeigt werden soll. Optionen: [!UICONTROL text field] |
| [!UICONTROL Payment Action] | Website | Die [Zahlungsaktion](https://experienceleague.adobe.com/en/docs/commerce-admin/config/sales/payment-methods/payment-methods) für die angegebene Zahlungsmethode. Optionen: [!UICONTROL Authorize] / [!UICONTROL Authorize and Capture] |
| [!UICONTROL Show Apple Pay on checkout page] | Website | Aktivieren oder Deaktivieren von [!DNL Apple Pay] auf der Kaufbestätigungsseite. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Apple Pay at start of checkout] | Shop-Ansicht | Aktivieren oder deaktivieren Sie [!DNL Apple Pay] zu Beginn des Checkout-Flusses. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Sort order] | Shop-Ansicht | Die Sortierreihenfolge für die angegebene Zahlungsmethode auf der Kasse. `Numeric Only` |
| [!UICONTROL Show Apple Pay on product detail page] | Shop-Ansicht | Aktivieren oder Deaktivieren von [!DNL Apple Pay] auf der Produktdetailseite. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Apple Pay in mini cart preview] | Shop-Ansicht | Aktivieren oder deaktivieren Sie [!DNL Apple Pay] in der Vorschau des Mini-Warenkorbs. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Apple Pay on cart page] | Shop-Ansicht | Aktivieren oder deaktivieren Sie [!DNL Apple Pay] in auf der Warenkorbseite. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Debug Mode] | Website | Aktivieren oder Deaktivieren des Debug-Modus. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
1. Erweitern Sie im linken Bereich **[!UICONTROL Sales]** und wählen Sie **[!UICONTROL Payment Methods]**.
1. Erweitern Sie den Abschnitt _[!UICONTROL FEATURED ADOBE PAYMENT SOLUTION]_.
1. Erweitern Sie im Abschnitt _[!UICONTROL [!DNL Payment Services]]_&#x200B;den Abschnitt&#x200B;_[!UICONTROL Google Pay]_ .
...
1. Um festzulegen, ob Kundinnen und Kunden nach der Bezahltabelle für Google eine separate **für die Google-Bezahlung sehen, setzen Sie &#x200B;** [!UICONTROL Skip Review]&#x200B;**auf `Yes` oder `No`.** Wenn auf `Yes` gesetzt, zeigen unterstützte Express-Flüsse **Versandmethoden in der Google-Zahlungsliste** (Client-seitige Versandrückrufe) an und können ohne den zusätzlichen Überprüfungsschritt abgeschlossen werden. Bei Festlegung auf `No` können Käufer den Versand und die Gesamtwerte auf der Überprüfungsseite bestätigen, bevor sie bezahlen.
1. Um **[!UICONTROL 3D Secure authentication]** zu aktivieren (standardmäßig `Off`), wählen Sie `Always` oder `When required`.
1. Um den Debugging-Modus zu aktivieren, wählen Sie `Yes` für die **[!UICONTROL Debug Mode]** aus (`No` deaktiviert ihn).
...
1. Um die Höhe festzulegen, verwenden Sie den Standardwert für die in **[!UICONTROL Button Style]** definierte Höhe.
1. Um Ihre Änderungen zu speichern, klicken Sie auf **[!UICONTROL Save Config]** .
1. Navigieren Sie zu **[!UICONTROL System]** > **[!UICONTROL Cache Management]** und klicken Sie dann auf **[!UICONTROL Flush Cache]** , um alle ungültigen Caches zu aktualisieren.
...
| [!UICONTROL Payment Action] | Website | Die [Zahlungsaktion](https://experienceleague.adobe.com/en/docs/commerce-admin/config/sales/payment-methods/payment-methods) für die angegebene Zahlungsmethode. Optionen: `[!UICONTROL Authorize]` / `[!UICONTROL Authorize and Capture]` |
| [!UICONTROL Show Google Pay on checkout page] | Website | Aktivieren oder Deaktivieren von [!DNL Google Pay] auf der Kaufbestätigungsseite. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Google Pay at start of checkout] | Shop-Ansicht | Aktivieren oder deaktivieren Sie [!DNL Google Pay] zu Beginn des Checkout-Flusses. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Sort order] | Shop-Ansicht | Die Sortierreihenfolge für die angegebene Zahlungsmethode auf der Kasse. `Numeric Only` |
| [!UICONTROL Show Google Pay on product detail page] | Shop-Ansicht | Aktivieren oder Deaktivieren von [!DNL Google Pay] auf der Produktdetailseite. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Google Pay in mini cart preview] | Shop-Ansicht | Aktivieren oder deaktivieren Sie [!DNL Google Pay] in der Vorschau des Mini-Warenkorbs. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Google Pay on cart page] | Shop-Ansicht | Aktivieren oder Deaktivieren von [!DNL Google Pay] auf der Warenkorbseite. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Skip Review] | Shop-Ansicht | Wenn auf `[!UICONTROL Yes]` festgelegt, können geeignete [!DNL Google Pay]-Express-Flüsse die separate Überprüfungsseite nach der Zahlungsliste weglassen. Versandmethoden werden in der Google-Zahlungsliste angezeigt. Wenn auf `[!UICONTROL No]` festgelegt, gehen Käufer zur Überprüfungsseite, um den Versand und die Gesamtwerte zu bestätigen. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
| [!UICONTROL Display Pay Later Message] | Website | Aktivieren oder deaktivieren Sie die PayPal-Nachricht Pay Later im Warenkorb, auf der Produktseite, im Mini-Warenkorb und während des Checkout-Flusses. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
| [!UICONTROL Show buttons at start of checkout] | Shop-Ansicht | Aktivieren oder deaktivieren Sie [!DNL PayPal payment buttons] zu Beginn des Checkout-Flusses. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Sort order] | Shop-Ansicht | Die Sortierreihenfolge für die angegebene Zahlungsmethode auf der Kasse. `Numeric Only` |
...
| [!UICONTROL App Switch] | Shop-Ansicht | Diese Option gilt nur für **USA** Händler und Käufer. Wenn diese Option aktiviert ist, werden alle Express PayPal-Zahlungen über die PayPal-Mobile-App ausgeführt, sofern verfügbar. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Contact Preference] | Shop-Ansicht | Diese Option gilt nur für **USA** Händler und Käufer. Wenn auf **Ja** gesetzt, werden die E-Mail-Adresse und Telefonnummer des Käufers im PayPal-Zahlungsmodal angezeigt. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
Lokale Zahlungsmethoden (Local Payment Methods, LPMs) unterstützen neben bestehenden kartenbasierten Optionen auch regionsspezifische und lokale Zahlungsmethoden wie Banküberweisungen und lokalisierte Zahlungslösungen. Händler können verfügbare LPMs direkt in der Commerce-Konfiguration aktivieren oder deaktivieren. Weitere Informationen [&#x200B; verfügbaren Methoden finden &#x200B;](payments-options.md#local-payment-methods) unter „Zahlungsoptionen“.

1. Navigieren Sie in _Admin_-Seitenleiste zu **[!UICONTROL Stores]** > _[!UICONTROL Settings]_>**[!UICONTROL Configuration]**.
1. Erweitern Sie im linken Bereich **[!UICONTROL Sales]** und wählen Sie **[!UICONTROL Payment Methods]**.
1. Erweitern Sie den Abschnitt _[!UICONTROL FEATURED ADOBE PAYMENT SOLUTION]_.
1. Erweitern Sie im Abschnitt _[!UICONTROL Payment Services]_&#x200B;den Abschnitt&#x200B;_[!UICONTROL Local Payment Methods]_ .
1. Wählen Sie **[!UICONTROL Active]** `Yes` aus, um LPMs zu aktivieren, oder `No`, um sie zu deaktivieren.
1. Geben Sie **[!UICONTROL Title]** den Text ein, der während des Checkouts als Name der Zahlungsmethode angezeigt werden soll. Dieser Titel wird auch im Auftragsraster angezeigt.
1. Wählen Sie **[!UICONTROL Allowed Payment Methods]** die Zahlungsmethoden aus, die Sie anbieten möchten. Die verfügbaren Methoden hängen von der Rechnungsadresse des Käufers und der Basiswährung der Website ab.
1. Geben Sie **[!UICONTROL Sort Order]** einen numerischen Wert ein, um die Anzeigeposition von LPMs innerhalb der Liste der verfügbaren Zahlungsmethoden zu bestimmen.
1. Um Ihre Änderungen zu speichern, klicken Sie auf **[!UICONTROL Save Config]**.
1. Navigieren Sie zu **[!UICONTROL System]** > **[!UICONTROL Cache Management]** und klicken Sie dann auf **[!UICONTROL Flush Cache]** , um alle ungültigen Caches zu aktualisieren.
...
| Feld | Umfang | Beschreibung |
|---|---|---|
| [!UICONTROL Active] | Website | Aktivieren oder Deaktivieren von lokalen Zahlungsmethoden. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Title] | Shop-Ansicht | Der Name der Zahlungsmethode, der beim Checkout und im Auftrags-Raster angezeigt wird. Optionen: [!UICONTROL text field] |
| [!UICONTROL Allowed Payment Methods] | Website | Auswahl der anzubietenden LPMs. Die Methoden sind nur für Kunden sichtbar, deren Rechnungsadresse und Website-Währung den Anforderungen der Methode entsprechen. Optionen: `Bancontact` / `BLIK` / `eps` / `iDEAL` / `MyBank` / `Przelewy24` |
| [!UICONTROL Sort Order] | Shop-Ansicht | Die Anzeigeposition der LPM-Gruppe innerhalb der Liste der verfügbaren Zahlungsmethoden. Diese Einstellung gilt für alle LPMs als Gruppe; einzelne LPMs können nicht unabhängig voneinander sortiert werden. `Numeric Only` |

>[!NOTE]
>
>Jede lokale Zahlungsmethode (LPM) hat spezifische Länder- und Währungsanforderungen. Eine Zahlungsmethode wird nur angezeigt, wenn das Land der Rechnungsadresse des Kunden und die Basiswährung der Website diesen Anforderungen entsprechen. Beispielsweise wird Bancontact nur für Kunden mit einer belgischen Rechnungsadresse angezeigt, wenn die Basiswährung EUR ist.

## Positionen

Positionen senden detaillierte Bestellinformationen an PayPal, einschließlich Produktdetails, Mengen und Preise. Diese Funktion ist standardmäßig aktiviert. Weitere Informationen finden [&#x200B; unter &#x200B;](line-items.md).

### Konfigurationsoptionen

| Feld | Umfang | Beschreibung |
|---|---|---|
| [!UICONTROL Line Items Enabled] | Website | Senden von Zeileneinträgen und Aufschlüsselung des Betrags an PayPal aktivieren oder deaktivieren. Deaktivieren Sie diese Einstellung, wenn Sie Erweiterungen von Drittanbietern verwenden, die benutzerdefinierte Gebühren hinzufügen, die von [!DNL Payment Services] nicht unterstützt werden. Optionen: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
Unter [Benutzerrollen](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/user-accounts/permissions-user-roles) erfahren Sie, wie Sie Rollen verwalten.
...
Weitere [&#x200B; zur Hierarchie von Websites, Stores und Store](https://experienceleague.adobe.com/en/docs/commerce-admin/start/setup/websites-stores-views)Ansichten finden Sie unter „Site, Store und View Scope“.
