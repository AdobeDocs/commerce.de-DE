...
In diesen Versionshinweisen werden die neuesten Versionen von [!DNL Payment Services] beschrieben.

Adobe unterstützt die neueste Version der Zahlungs-Services. Versionshinweise für ältere Versionen werden als Referenz bereitgestellt.
...
![Neues Problem](../assets/new.svg)<!-- Issue PAY-6374 --> Die Schaltfläche **Einstellungen** im [!DNL Payment Services]-Dashboard wird jetzt zur Seite „Admin-Konfiguration“ für Zahlungsmethoden weitergeleitet, was einen optimierten Workflow für die Verwaltung von Zahlungseinstellungen bietet.

_19. Januar 2026_

![Neues Problem](../assets/new.svg)<!-- Issue PAY-6325 --> Jetzt können Händler die PayPal-Verkäufer-ID als Spalte im [Transaktionsbericht“ anzeigen, &#x200B;](reporting.md#transactions-report-view) Transaktionen zu identifizieren, die ein bestimmter Kunde platziert hat.
...
![Neu](../assets/new.svg)<!-- Issue PAY-4050 --> Es wurde eine [Datenvisualisierungsansicht](payouts.md#payouts-data-visualization-view) hinzugefügt, die auf der Zahlungsdienst-Startseite für den Bericht „Zahlungsstatus der Bestellung“ angezeigt wird.
...
![Neu](../assets/new.svg)<!-- Issue PAY-3705 --> Die `increment_id` ist jetzt für die Abstimmung von Auszahlungen in externen ERP-Systemen verfügbar. Sie wird an die `invoice_id` `custom_id` _und_ übertragen, die sowohl im PayPal-Webhook als auch in den Details zur Händleraktivität für eine Auszahlung sichtbar sind.
...
![Neu](../assets/new.svg)<!-- Issue PAY-1720 --> Streitigkeiten bei Lagerbestellungen sind jetzt im Bericht [Status der Bestellzahlung“ &#x200B;](order-payment-status.md#view-disputes). Sie können Streitigkeiten beilegen, indem Sie von [!DNL Payment Services] direkt zum PayPal-Lösungszentrum gehen.
...
![Neu](../assets/new.svg)<!-- Issue PAY-2761 --> Sie können jetzt die Daten anpassen, die im Bericht [Zahlungsstatus bestellen](order-payment-status.md#show-and-hide-columns) und im Bericht [Auszahlungen](payouts.md#show-and-hide-columns) angezeigt werden, indem Sie Spalten mithilfe des Steuerelements Spalteneinstellungen ein- oder ausblenden.
...
![Neu](../assets/new.svg)<!-- PAY-6400 --> Es wurde eine **[!UICONTROL Line Items Enabled]** Konfiguration hinzugefügt, um den Versand von Zeileneinträgen und die Aufschlüsselung des Betrags an PayPal zu aktivieren oder zu deaktivieren. Diese Einstellung hilft beim Beheben von Checkout-Problemen, wenn Erweiterungen von Drittanbietern benutzerdefinierte Gebühren hinzufügen, die von [!DNL Payment Services] nicht unterstützt werden. Weitere Informationen finden [&#x200B; unter &#x200B;](line-items.md).

![Neu](../assets/new.svg)<!-- PAY-6457 --> Hinzugefügt [Express-Zahlungs-Schaltflächen zu Beginn des Checkouts](payments-options.md#express-checkout-buttons) um schnellere Checkouts zu fördern.

![Neu](../assets/new.svg)<!-- PAY-6458 --> Es wurde Unterstützung für [Lokale Zahlungsmethoden (LPMs)](payments-options.md#local-payment-methods) für regionsspezifische und lokale Zahlungsoptionen hinzugefügt. Händler können verfügbare LPMs direkt in [Commerce aktivieren oder &#x200B;](configure-admin.md#local-payment-methods).

![Neu](../assets/new.svg)<!-- PAY-6377 --> Es wurde eine Sandbox-Konfiguration [Land des Käufers](sandbox.md#buyers-country) hinzugefügt, um den Standort des Käufers zu simulieren und zu steuern, welche Zahlungsmethoden gerendert werden.

![Problem behoben](../assets/fix.svg)<!-- PAY-6458 --> Verbesserte Kompatibilität mit den Betrugsfiltern von PayPal durch Weitergabe von E-Mail-Adressdaten des Käufers und verbesserte Funktionen zum Schutz vor Betrug.

![Problem behoben](../assets/fix.svg)<!-- PAY-6413 --> Es wurde ein Problem behoben, bei dem der PayPal-Checkout für virtuelle Produkte aufgrund einer fehlenden Versandadresse nicht abgeschlossen werden konnte.

![Problem behoben](../assets/fix.svg)<!-- PAY-6450 --> Es wurde ein Problem behoben, bei dem PayPal-Button-Checkout-Fehler auf Produktdetailseiten, im Minicart, Warenkorb und am Anfang des Checkouts nicht korrekt verarbeitet wurden.

![Problem behoben](../assets/fix.svg)<!-- PAY-6453 --> Deaktivierte Server-seitige Versandrückrufe fließen beim GraphQL-Checkout, um Konflikte beim Checkout zu vermeiden.

![Problem behoben](../assets/fix.svg)<!-- PAY-6456 --> Die Konfigurationsbeschreibung von [Apple Pay](configure-admin.md#apple-pay) wurde aktualisiert, um klarzustellen, dass Apple Zahlungen von nicht verifizierten Domains ablehnt. Händler müssen alle Domains überprüfen, in denen die Apple-Pay-Buttons verwendet werden. Es gelten Länderbeschränkungen.

## v2.13.3

_14. Januar 2026_

![Problem behoben](../assets/fix.svg)<!-- PAY-6399 --> Es wurde ein Problem behoben, bei dem während eines Admin-Checkouts die falsche Vault-Karte verwendet wurde. Jetzt wird die ausgewählte Karte bei der Bestellung korrekt verwendet.

## v2.13.2

_5. Januar 2026_

![Es wurde ein Problem behoben](../assets/fix.svg)<!-- PAY-6390 --> Allgemeine Verbesserungen und Optimierungen durch die Implementierung einer verbesserten JS-Minimierung.
...
![Es wurde &#x200B;](../assets/fix.svg)<!-- PAY-6164 --> Problem behoben. Jetzt verwendet [!DNL Payment Services] die Basiswährung für die verfügbaren Versandmethoden im [Server-Side Shipping Callback (SSSC)](https://developer.paypal.com/docs/multiparty/checkout/standard/customize/shipping-module/).
...
![Neu](../assets/new.svg)<!-- PAY-6022 --> [Fastlane](https://experienceleague.adobe.com/de/docs/commerce/payment-services/payments-checkout/payments-options) bietet einen schnelleren Kauf während des Gast-Checkouts.
...
![Neu](../assets/new.svg)<!-- PAY-6227 --> Beim Auschecken mit PayPal überspringt [!DNL Payment Services] das Popup zur Bestellbestätigung, um einen schnelleren Kaufvorgang zu ermöglichen. Dies wird durch den [Server-seitigen Versandrückruf](payments-options.md#server-side-shipping-callbacks-for-paypal-payment-buttons) aktiviert, der den Versand und die Gesamtwerte direkt auf der PayPal-Prüfungsseite berechnet.
...
![Neu](../assets/new.svg)<!-- PAY-4868 --> Händler können [Google Pay während des gesamten Kauferlebnisses konfigurieren](payments-options.md) ähnlich wie andere Zahlungsschaltflächen in [!DNL Payment Services] über den Administrator.

![Neu](../assets/new.svg)<!-- PAY-4381 --> [Payment Services unterstützt Google Pay über GraphQL](https://developer.adobe.com/commerce/webapi/graphql/payment-services-extension/) sodass Händler ein Headless-Commerce-Erlebnis mit der Google Pay-Zahlungsmethode haben.

![Neu](../assets/new.svg)<!-- PAY-4878 --> Jetzt ist die [!DNL Payment Services] grundlegende Checkout-Funktion für Adobe Commerce- und Magento Open Source-Händler gebündelt.[!DNL Payment Services] kann jetzt Händler mit Unternehmen in 200 Regionen weltweit unterstützen.[!DNL Payment Services] Beim einfachen Checkout stehen die Optionen „Debit/Credit“, „PayPal“, „Venmo“ (falls verfügbar) und „PayLater“ (falls verfügbar) in einem Self-Service-Onboarding zur Verfügung.

![Behebung](../assets/fix.svg)<!-- PAY-5291 --> Bei einigen Transaktionen kann sich der Erhalt einer Zahlungsbestätigung verzögern. In diesem Fall können Händler jetzt einen aktualisierten Zahlungsstatus für eine Bestellung erhalten. [Zahlungsdienste erkennen den Status „Ausstehend“ einer &#x200B;](order-payment-status.md#payment-status-updates) in einer Bestellung, indem sie ausstehende Transaktionen erkennen und diese Transaktionen proaktiv überwachen und aktualisieren, wenn der Status „Ausstehend“ erfasst wurde.
...
- In der [&#x200B; „Auftragsdetails“ &#x200B;](https://experienceleague.adobe.com/de/docs/commerce-admin/stores-sales/order-management/orders/order-processing#view-an-order) Commerce Admin.
...
![Neu](../assets/new.svg)<!-- Issue xxx --> Hinzugefügt [Verfügbarkeit in folgenden Ländern und assoziierten Währungen](compatibility.md#standard-vs-advanced-payment-services-experience): Australien, Frankreich, Großbritannien.
...
![Behebung](../assets/fix.svg)<!-- Issue PAY-4226 --> Es wurde ein Problem behoben, das neue [!DNL Payment Services] daran hinderte, den Checkout in der Admin zu verwenden.[!DNL Payment Services] verwendete zuvor die Commerce-Kunden-ID, die für neue Kundinnen und Kunden nicht vorhanden ist.
...
![Neu](../assets/new.svg)<!-- Issue PAY-3950 --> Händler können jetzt die Funktion [Instant Purchase Commerce](https://experienceleague.adobe.com/de/docs/commerce-admin/stores-sales/point-of-purchase/checkout-instant-purchase) für ihre Stores aktivieren, sodass Käufer ([Vault-Kreditkarteninformationen) &#x200B;](vaulting.md) können, um den Checkout zu beschleunigen.
...
![Bekanntes Problem](../assets/bug.svg)<!-- Issue PAY-0 --> [!DNL Payment Services] ist nicht kompatibel mit `commerce-data-export` Version 101.20 und höher, weshalb sie nicht mehr kompatibel mit der [[!DNL Channel manager] Erweiterung](https://experienceleague.adobe.com/de/docs/discontinued/using/commerce-channels) (eingestellt) ist.
...
![Neu](../assets/new.svg)<!-- Issue PAY-2682 --> Die [!DNL Payment Services]-Erweiterung für [!DNL Adobe Commerce] und [!DNL Magento Open Source] ist jetzt für kanadische Händler verfügbar. Händler können die Zahlungskonfiguration entweder auf [Französisch](compatibility.md##standard-vs-advanced-payment-services-experience) oder [Englisch](compatibility.md#standard-vs-advanced-payment-services-experience) anzeigen.

![Neu](../assets/new.svg)<!-- Issue PAY-2681 --> [!DNL Payment Services] unterstützt [Canadian Dollar (CAD)](compatibility.md#standard-vs-advanced-payment-services-experience) für Kreditkarten und PayPal-Transaktionen.
...
![Problem behoben](../assets/fix.svg)<!-- Issue PAY-2277 --> Sie können jetzt einzelne PayPal[Zahlungsschaltflächen selektiv aktivieren oder deaktivieren](configure-admin.md#paypal-payment-buttons).
...
![Bekanntes Problem](../assets/bug.svg)<!-- Issue PAY-2842 --> Testen von Kreditkartentransaktionen [kann mit PayPal fehlschlagen](https://experienceleague.adobe.com/de/docs/experience-cloud-kcs/kbarticles/ka-26825) wenn Zahlungen in einer Sandbox-Umgebung verarbeitet werden.
...
![Bekanntes Problem](../assets/bug.svg)<!-- Issue PAY-2473 --> Die Verwendung [falschen Composer-Schlüsseln](https://experienceleague.adobe.com/de/docs/experience-cloud-kcs/kbarticles/ka-26826) während der Installation der Erweiterung verhindert, dass der Benutzer sich [&#x200B; mit &#x200B;](https://experienceleague.adobe.com/de/docs/commerce-operations/installation-guide/prerequisites/authentication-keys) richtigen `MAGEID` authentifiziert.

![Bekanntes Problem](../assets/bug.svg)<!-- Issue PAY-2474 --> [!DNL Payment Services] Berichte [werden möglicherweise nicht sofort synchronisiert](https://experienceleague.adobe.com/de/docs/experience-cloud-kcs/kbarticles/ka-26837).

![Bekanntes Problem](../assets/bug.svg)<!-- Issue PAY-2475 --> Ihr [PayPal-Sandbox-](https://experienceleague.adobe.com/de/docs/experience-cloud-kcs/kbarticles/ka-26836) für [!DNL Payment Services] kann nicht überprüft werden, wenn Sie dieses Konto beim Onboarding erstellen.
