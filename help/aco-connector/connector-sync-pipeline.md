...
1. **Erkennung von Entitätsänderungen** - (alle 1 Minute) Ein Cron-Auftrag (`indexer_reindex_all_invalid`) erkennt [!DNL Adobe Commerce] Entitätsänderungen und Trigger des [!DNL SaaS Data Export], der Feed-Elemente zusammenstellt.
...
1. **Ergebnisse beibehalten** - Beibehalten des API-Antwortstatus in den [Feed-Tabellen](reference/connector-reference.md#supported-feeds).
...
Die folgenden Cron-Aufträge automatisieren die Pipeline nach einem festen Zeitplan.
...
- [Commerce Cron muss ausgeführt werden](https://experienceleague.adobe.com/en/docs/commerce-knowledge-base/kb/troubleshooting/miscellaneous/cron-readiness-check-issues){target="_blank"}.
