...
description: Erfahren Sie, wie Sie zu Adobe Commerce as a Cloud Service migrieren.
...
Adobe Commerce as a Cloud Service bietet die meisten vorkonfigurierten Konfigurationen. Wenn Sie jedoch von einer bestehenden Adobe Commerce in der Cloud oder einer lokalen Instanz migrieren, müssen Sie je nach Ihrer spezifischen Konfiguration verschiedene Migrationsaktionen durchführen.

## Migrationspfade

Adobe Commerce as a Cloud Service unterstützt mehrere Migrationspfade, abhängig von Ihrer Timeline, Storefront und Ihren Anpassungen.

Als Alternative zu einer vollständigen Migration unterstützt Adobe Commerce as a Cloud Service eine stufenweise Migration mithilfe von Commerce Optimizer oder eines inkrementellen Ansatzes.

* **Inkrementelle Migration** Dieser Ansatz umfasst die schrittweise Migration Ihrer Daten, Anpassungen und Integrationen. Dieser Ansatz eignet sich ideal für große Händler mit vielen Anpassungen, die ihre komplexen Anpassungen und Daten schrittweise in ihrem eigenen Tempo auf Adobe Commerce as a Cloud Service umstellen möchten.

![Inkrementelle Migration](./assets/incremental.png){width="600" zoomable="yes"}

* **Commerce Optimizer**: Bei diesem Ansatz können Sie iterativ migrieren, indem Sie Commerce Optimizer als Übergangsphase verwenden, um komplexe Anpassungen und Daten in Ihrem eigenen Tempo auf Adobe Commerce as a Cloud Service zu übertragen. Commerce Optimizer bietet Zugriff auf Merchandising-Services, die auf Katalogkanälen und Richtlinien basieren, auf die Commerce-Storefront mit Edge Delivery und auf Produktvisualisierungen mit AEM Assets.

![Iterative Migration](./assets/optimizer.png){width="600" zoomable="yes"}

* **Vollständige Migration** Dieser Ansatz umfasst die Migration aller Daten, Anpassungen und Integrationen auf einmal. Dieser Ansatz eignet sich ideal für kleinere Händler mit wenigen Anpassungen, die schnell auf Adobe Commerce as a Cloud Service umstellen möchten.
...
* **Datenmigration** - Verwenden der bereitgestellten Migrations-Tools, um Daten von Ihrer bestehenden Instanz zu Adobe Commerce as a Cloud Service zu migrieren.
...
* **Integrationen** - Alle Integrationen müssen entweder das [Integrations-Starter-Kit](https://developer.adobe.com/commerce/extensibility/starter-kit/integration/) oder die [Adobe Commerce as a Cloud Service REST-API nutzen](https://developer.adobe.com/commerce/services/reference/cloud-service/core-admin/).
...
* **Erweiterungen** - Alle prozessinternen Erweiterungen müssen als prozessexterne Erweiterungen neu erstellt werden. Bis Ende 2025 wird Adobe 100 unserer beliebtesten Erweiterungen vorgeladen und einsatzbereit bereitstellen, um die Build-Zeiten zu minimieren.

## Migrationsphasen

Die Migration von Ihrer aktuellen Adobe Commerce-Instanz zu einer neuen Adobe Commerce as a Cloud Service-Instanz umfasst hauptsächlich die folgenden Phasen:
...
1. Überprüfen Sie zunächst die Architektur, das Erweiterbarkeits-Framework und die Storefront-Funktionen von Adobe Commerce as a Cloud Service:
...
1. Überprüfen Sie Ihre aktuellen Drittanbieterintegrationen und bestätigen Sie die API-Kompatibilität mit der Adobe Commerce as a Cloud Service-Plattform.

### Implementierungsphase

Die folgenden Schritte beschreiben den Entwicklungs- und Ausführungsprozess der Migration:

1. Erstellen Sie eine neue Adobe Commerce as a Cloud Service-Instanz im [Commerce Cloud Manager](./getting-started.md#create-an-instance).

1. Installieren erforderlicher Apps und Anpassungen. Adobe Commerce as a Cloud Service ist mit 100 der beliebtesten Apps vorgeladen. Wenn Sie zusätzliche Apps oder Anpassungen benötigen, können Sie diese mit App Builder erneut implementieren.
...
Validieren und testen Sie Ihre neue Adobe Commerce as a Cloud Service-Instanz, bevor Sie sie starten:
...
Nachdem Sie Ihre neue Adobe Commerce as a Cloud Service-Sandbox-Instanz validiert und getestet haben, können Sie Ihre Produktionsinstanz starten:
